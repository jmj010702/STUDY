...
- 사내 호출 구조: Redis 캐시 → 미스 시 RestClient 의 2단 구조
- 캐시가 비는 순간(TTL 1시간 만료 · 재배포 직후 · 신규 ID 첫 조회)에만 RestClient 가 실제 호출되며 실패
...
- 정상 동작 호출(face-api, redis, kafka) 비교 → 모두 K8s Service DNS + 포트 직접 사용, `@LoadBalanced` 미사용
- "K8s Service 가 이미 서버사이드 LB 수행, 클라이언트 LB 중복" 결론 도출
